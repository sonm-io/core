...
func (p *tokenPriceWatcher) Update(ctx context.Context) error {
	...
	for _, t := range tokenPrices {
		if _, ok := tokensToWorkWithMap[t.Symbol]; ok {
			...
			if err != nil {
				return errors.Wrapf(err, "cannot parse USD price from \"%v\"", t.PriceUsd)
			}
	...
	for _, token := range tokensToWorkWith {
		...
		if !ok {
			return fmt.Errorf("cannot get token by symbol \"%s\": %v", tokensSnapshot.Data.General.Symbol, err)
		}
