...
func TestDWH_GetOrders(t *testing.T) {
	...
	// Test TEXT columns.
	{
		...
		if reply.Orders[0].GetOrder().AuthorID != common.HexToAddress("0xA").Hex() {
...
func TestDWH_GetOrderDetails(t *testing.T) {
	...
	if reply.AuthorID != common.HexToAddress("0xA").Hex() {
		t.Errorf("Expected %s, got %s (AuthorID)", common.HexToAddress("0xA").Hex(), reply.AuthorID)
	}
	if reply.CounterpartyID != common.HexToAddress("0xB").Hex() {
...
func TestDWH_monitor(t *testing.T) {
	...
	order := &pb.Order{
		...
		AuthorID:       common.HexToAddress("0xD").Hex(),
	...
	certificate := &pb.Certificate{
		ValidatorID:   common.HexToAddress("0xC").Hex(),
		OwnerID:       common.HexToAddress("0xD").Hex(),
