...
var DealOpenedTopicDeprecated = common.HexToHash("0x873cb35202fef184c9f8ee23c04e36dc38f3e26fb285224ca574a837be976848")
...
func (bch *api) checkTransactionResult(ctx context.Context, tx *types.Transaction) (*big.Int, error) {
	...
	for _, l := range txReceipt.Logs {
		...
		topicMatched := (nameTopic == DealOpenedTopicDeprecated) || (nameTopic == DealAcceptedTopicDeprecated) || (nameTopic == DealClosedTopicDeprecated)
...
func (bch *api) GetOpenedDeal(ctx context.Context, hubAddr string, clientAddr string) ([]*big.Int, error) {
	...
	var eventTopic = []common.Hash{DealOpenedTopicDeprecated, DealAcceptedTopicDeprecated, DealClosedTopicDeprecated}
	...
	for _, l := range logs {
		...
		switch l.Topics[0] {
		case DealOpenedTopicDeprecated:
			idsOpened = append(idsOpened, idTopic)
			break
		case DealAcceptedTopicDeprecated, DealClosedTopicDeprecated:
...
func (bch *api) GetAcceptedDeal(ctx context.Context, hubAddr string, clientAddr string) ([]*big.Int, error) {
	...
	// precompile EventName topics
	var eventTopic = []common.Hash{DealAcceptedTopicDeprecated, DealClosedTopicDeprecated}
	topics = append(topics, eventTopic)
	...
	for _, l := range logs {
		...
		switch l.Topics[0] {
		case DealAcceptedTopicDeprecated:
			idsOpened = append(idsOpened, idTopic)
			break
		case DealClosedTopicDeprecated:
			mb[idTopic.String()] = true
			break
		}
	}
...
func (bch *api) GetClosedDeal(ctx context.Context, hubAddr string, clientAddr string) ([]*big.Int, error) {
	...
	// precompile EventName topics
	var eventTopic = []common.Hash{DealClosedTopicDeprecated}
	topics = append(topics, eventTopic)
