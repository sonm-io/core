...
contract('FixedPoint', (accounts) => {
    ...
    describe('Round', (accounts) => {
        it('should properly round numbers', async () => {
            let fp = fp10;
        ...
        it('should properly construct fp numbers from natural numbers', async () => {
            let fp = fp248;
            let mul = await fp.FromNatural(255);
            let round = (await fp.Round(mul)).toNumber();
            assert.equal(round, 255);
            await assertRevert(fp.FromNatural(256));
        });

        it('should properly sum numbers and detect overflow', async () => {
            let fp = fp248;
            ...
            assert.equal(round, 255);
            await assertRevert(fp.Add(val2, val2));
            let one = (await fp.FromNatural(1));
            await assertRevert(fp.Add(sum, one));
        });

        it('should properly sub numbers and detect underflow', async () => {
            let fp = fp248;
            let val1 = await fp.FromNatural(127);
            let val2 = await fp.FromNatural(128);
            ...
            await assertRevert(fp.Sub(val1, val2));
        });

        it('should properly multiply numbers', async () => {
            let fp = fp248;
            let precision = await fp.Precision();
            ...
            await assertRevert(fp.Mul(val2, val2));
        });

        it('should properly divide numbers', async () => {
            let fp = fp10;
        ...
        it('should properly make integer power', async () => {
            let fp = fp10;
