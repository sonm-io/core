...
contract('ProfileRegistry', async function (accounts) {
    ...
    const test_validator = accounts[3];
    ...
    describe('AddValidator', function () {
        ...
        it('add new validator and spend `ValidatorCreated` event', async function () {
            const {logs} = await registry.AddValidator(test_validator, test_level, {from: owner});
            assert.equal(logs.length, 1);
            assert.equal(logs[0].event, 'ValidatorCreated');
            assert.equal(logs[0].args.validator, test_validator);
        });

        it('with preffred lvl', async function () {
            let validator = await registry.GetValidatorLevel(test_validator);
            assert.equal(validator.toNumber(), test_level);
        });

        it('add new validator twice - should revert', async function () {
            await assertRevert(registry.AddValidator(test_validator, test_level, {from: owner}));
        });

        describe('when not owner want to add validator', function () {
            it('should revert', async function () {
                await assertRevert(registry.AddValidator(master, test_level, {from: creeper}));
            });
        });
    ...
    describe('RemoveValidator', function () {
        before(async function () {
            ...
            await registry.AddValidator(test_validator, test_level, {from: owner});
        });

        it('remove validator and spend `ValidatorDeleted` event', async function () {
            await registry.RemoveValidator(test_validator, {from: owner});
        });
        ...
        describe('when validator want to remove itself', function () {
            ...
            it('should revert', async function () {
                await assertRevert(registry.RemoveValidator(test_validator, {from: creeper}));
            });
    ...
    describe('CreateCertificate', function () {
        ...
        describe('when certificate has single attribute', function () {
            ...
            describe('when validator create certificate with same value', function () {
                ...
                before(async function () {
                    const {logs} = await registry.CreateCertificate(master, test_type, 'value', {from: validatorLvl2});
                    const value = await registry.GetAttributeValue(master, test_type);
                    assert.equal(web3.toAscii(value), 'value');

                    firstCertificateId = logs[0].args.id;
                    start_value = await registry.GetAttributeValue(master, test_type);
                    start_count = await registry.GetAttributeCount(master, test_type);
                });

                it('should created', async function () {
                    const {logs} = await registry.CreateCertificate(master, test_type, 'value', {from: validatorLvl3});
                ...
                it('value cant changed', async function () {
                    const value = await registry.GetAttributeValue(master, test_type);
                    assert.equal(web3.toAscii(value), 'value');
                    assert.equal(value, start_value);
                });

                it('attribute count should increase', async function () {
                    const count = await registry.GetAttributeCount(master, test_type);
                    assert.equal(count.toNumber(), parseInt(start_count) + 1);
                });
            ...
            describe('when validator create certificate with other value', function () {
                ...
                before(async function () {
                    const {logs} = await registry.CreateCertificate(master, test_type, test_value, {from: validatorLvl2});
                    const value = await registry.GetAttributeValue(master, test_type);
                    assert.equal(web3.toAscii(value), test_value);

                    firstCertificateId = logs[0].args.id;
                });

                it('should revert', async function () {
                    await assertRevert(registry.CreateCertificate(master, test_type, test_value + test_value, {from: validatorLvl3}));
                });
    ...
    describe('GetCertificate', function () {
        ...
        it('should return existing certificate with correct values', async function () {
            ...
            assert.equal(result[2], test_type);
            assert.equal(web3.toAscii(result[3]), test_value);
        });
    ...
    describe('RemoveCertificate', function () {
        ...
        describe('when certificate had single attribute with two confirmations', function () {
            ...
            before(async function () {
                let {logs} = await registry.CreateCertificate(master, test_type, test_value, {from: validatorLvl2});
                firstCertId = logs[0].args.id;

                let tx = await registry.CreateCertificate(master, test_type, test_value, {from: validatorLvl4});
                secondCertId = tx.logs[0].args.id;

                started_value = await registry.GetAttributeValue(master, test_type);
                assert.equal(web3.toAscii(started_value), test_value);
                started_count = await registry.GetAttributeCount(master, test_type);
            });
            ...
            describe('when first certificate removed', function () {
                ...
                it('attribute value should not changed', async function () {
                    let value = await registry.GetAttributeValue(master, test_type);
                    assert.equal(web3.toAscii(value), web3.toAscii(started_value));
                });

                it('attribute count should decrease by 1', async function () {
                    let count = await registry.GetAttributeCount(master, test_type);
                    assert.equal(count.toNumber(), started_count.toNumber() - 1);
                });
            });
        ...
        describe('when certificate has multiple attribute', function () {
            ...
            before(async function () {
                ...
                started_value = await registry.GetAttributeValue(master, test_type);
                assert.equal(web3.toAscii(started_value), '');
                started_count = await registry.GetAttributeCount(master, test_type);
            });
