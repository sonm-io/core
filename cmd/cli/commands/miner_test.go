...
func TestMinerListEmpty(t *testing.T) {
	itr := NewMockCliInteractor(gomock.NewController(t))
	itr.EXPECT().MinerList(gomock.Any()).AnyTimes().Return(&pb.H_MinerListReply{}, nil)
...
func TestMinerListData(t *testing.T) {
	...
	itr.EXPECT().
		...
		AnyTimes().
		Return(&pb.H_MinerListReply{
			Info: map[string]*pb.H_MinerListReply_ListValue{
				"test": {
