...
type CliInteractor interface {
	...
	HubSlotsShow(ctx context.Context, ID string) (*pb.GetSlotsReply, error)
...
func (it *grpcInteractor) HubSlotsShow(ctx context.Context, ID string) (*pb.GetSlotsReply, error) {
	c, cancel := it.ctx(ctx)
	defer cancel()
